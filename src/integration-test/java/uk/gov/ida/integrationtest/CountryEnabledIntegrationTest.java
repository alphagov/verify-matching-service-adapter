...
public class CountryEnabledIntegrationTest {
    ...
    public void shouldProcessEidasAttributeQueryRequest() {
        ...
        AttributeQuery attributeQuery = AttributeQueryBuilder.anAttributeQuery()
            ...
            .withSubject(
                    aSubjectWithEncryptedAssertions(
                        singletonList(anEidasEncryptedAssertion(issuerId)), REQUEST_ID, HUB_ENTITY_ID)
                )
                .withSignature(
                    aSignature()
        ...
        assertThat(response.readEntity(String.class)).contains("TODO: Signature valid. Next stage of eIDAS MSA development is to validate the AQR");
    }

    @Test
    public void shouldNotProcessAnAttributeQueryRequestWithAnInvalidSignature() {
        String issuerId = STUB_IDP_ONE;
        AttributeQuery attributeQuery = AttributeQueryBuilder.anAttributeQuery()
            .withId(REQUEST_ID)
            .withIssuer(anIssuer().withIssuerId(issuerId).build())
            .withSubject(
            ...
            .withSignature(
                aSignature()
                    .withSigningCredential(
                        new TestCredentialFactory(
                            TestCertificateStrings.HEADLESS_RP_PUBLIC_SIGNING_CERT,
        ...
        assertThat(response.getStatus()).isEqualTo(HttpStatus.SC_INTERNAL_SERVER_ERROR);
        assertThat(response.readEntity(String.class)).contains(invalidMessageSignature().getErrorMessage());
    }
