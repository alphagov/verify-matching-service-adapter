...
public class SubjectConfirmationValidator<T> extends CompositeValidator<T> {
    ...
    public SubjectConfirmationValidator(Function<T, SubjectConfirmation> valueProvider, TimeRestrictionValidator timeRestrictionValidator, String requestId) {
        super(
            true,
            valueProvider,
            new FixedErrorValidator<>(sc -> !sc.getMethod().equals(METHOD_BEARER),  WRONG_SUBJECT_CONFIRMATION_METHOD),
            new SubjectConfirmationDataValidator<>(SubjectConfirmation::getSubjectConfirmationData, timeRestrictionValidator, requestId)
        );
    }
